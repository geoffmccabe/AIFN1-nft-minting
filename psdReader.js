...
export function validSignatureAt(reader, offset) {
  const sig =
    String.fromCharCode(reader.view.getUint8(offset)) +
...
export function readPsd(reader, readOptions = {}) {
  ...
  const skipComposite =
    reader.skipCompositeImageData && (reader.skipLayerImageData || hasChildren)
...
function readLayerMaskData(reader, layer) {
  return readSection(reader, 1, left => {
    ...
    if (left() >= 18) {
      ...
      realMask.positionRelativeToLayer =
        (realFlags & LayerMaskFlags.PositionRelativeToLayer) !== 0
      realMask.disabled = (realFlags & LayerMaskFlags.LayerMaskDisabled) !== 0
      realMask.fromVectorData =
        (realFlags & LayerMaskFlags.LayerMaskFromRenderingOtherData) !== 0
...
function readLayerChannelImageData(reader, psd, layer, channels) {
  ...
  for (const channel of channels) {
    ...
    if (
      channel.id === ChannelID.UserMask ||
      channel.id === ChannelID.RealUserMask
    ) {
      const mask =
        channel.id === ChannelID.UserMask ? layer.mask : layer.realMask
      if (!mask)
        throw new Error(
          `Missing layer ${
            channel.id === ChannelID.UserMask ? "mask" : "real mask"
          } data`
        )
...
export function readAdditionalLayerInfo(reader, target, psd, imageResources) {
  ...
  const u64 =
    sig === "8B64" ||
    (reader.large && largeAdditionalInfoKeys.indexOf(key) !== -1)
    ...
    left => {
      ...
      if (handler) {
        ...
        reader.logMissingFeatures &&
          reader.log(`Unhandled additional info: ${key}`)
        skipBytes(reader, left())
      }

      if (left()) {
        reader.logMissingFeatures &&
          reader.log(`Unread ${left()} bytes left for additional info: ${key}`)
        skipBytes(reader, left())
      }
    },
    false,
