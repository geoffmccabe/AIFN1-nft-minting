import { ColorMode, SectionDividerType } from "./psd"
import {
  hasAlpha,
  createCanvas,
  writeDataRLE,
...
function verifyBitCount(target) {
  ...
  if (
    data &&
    (data.data instanceof Uint32Array || data.data instanceof Uint16Array)
  ) {
...
export function writePsd(writer, psd, options = {}) {
  ...
  if (!imageData && psd.canvas) {
    imageData = psd.canvas
      .getContext("2d")
      .getImageData(0, 0, psd.canvas.width, psd.canvas.height)
  }

  if (
    imageData &&
    (psd.width !== imageData.width || psd.height !== imageData.height)
  )
  ...
  imageResources.layerGroupsEnabledId = layers.map(l =>
    l.linkGroupEnabled == false ? 0 : 1
  )
  ...
  writeSection(
    ...
    () => {
      ...
      const layersData = layers.map((l, i) =>
        getChannels(writer.tempBuffer, l, i === 0, options)
      )
      ...
      for (const layerData of layersData) {
        ...
        if (
          ...
          (layer.sectionDivider &&
            layer.sectionDivider.type !== SectionDividerType.Other) ||
          layer.adjustment
        ) {
...
function getMaskChannels(
  ...
  if (width && height && imageData) {
    ...
    if (RAW_IMAGE_DATA && layer[realMask ? "realMaskDataRaw" : "maskDataRaw"]) {
      ...
      compression =
        layer[
          realMask ? "realMaskDataRawCompression" : "maskDataRawCompression"
        ]
...
function getLayerChannels(tempBuffer, layer, background, options) {
  ...
  let data =
    layer.imageData ||
    layer.canvas.getContext("2d").getImageData(0, 0, width, height)
  ...
  if (options.trimImageData) {
    ...
    if (
      trimmed.left !== 0 ||
      trimmed.top !== 0 ||
      trimmed.right !== data.width ||
      trimmed.bottom !== data.height
    ) {
      left += trimmed.left
      top += trimmed.top
  ...
  if (
    ...
    layer.mask ||
    hasAlpha(data) ||
    (RAW_IMAGE_DATA && layer.imageDataRaw?.["-1"])
  ) {
