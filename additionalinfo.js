...
addHandler(
  ...
  (writer, target, { width, height }) => {
    ...
    for (const path of vectorMask.paths) {
      ...
      writeUint16(
        writer,
        path.operation ? booleanOperations.indexOf(path.operation) : -1
      ) // -1 for undefined
    ...
    for (let i = 0; i < count; i++) {
      ...
      readSection(reader, 1, left => {
        if (key === "cust") {
          ...
          desc // TODO: save this
          reader.logMissingFeatures &&
            reader.log('Unhandled "shmd" section key', key)
        } else {
          reader.logMissingFeatures &&
            reader.log('Unhandled "shmd" section key', key)
        }
...
function parseFilterFX(desc, options) {
  return {
    ...
    list: desc.filterFXList
      .map(x => parseFilterFXItem(x, options))
      .filter(x => !!x)
  }
}
    ...
    case "curves":
      return {
        ...
        Fltr: {
          ...
          Adjs: f.filter.adjustments.map(a =>
            "curve" in a
              ? {
                  _name: "",
                  _classID: "CrvA",
...
addHandler(
  "Anno",
  target => target.annotations !== undefined,
  (reader, target, left) => {
  ...
  addHandler(
    ...
    (reader, target, left, _psd) => {
      ...
      while (left() > 8) {
        ...
        if (type === "liFE" && version > 3) {
          ...
          file.time = new Date(
            Date.UTC(year, month, day, hour, minute, wholeSeconds, ms)
          ).toISOString()
        }
    ...
    (writer, target) => {
      ...
      for (const file of psd.linkedFiles) {
        ...
        writeSignature(
          writer,
          tag === "lnkE" ? "liFE" : file.data ? "liFD" : "liFA"
        )
  ...
  (reader, target, leftBytes) => {
    ...
    while (leftBytes() > 8) {
      ...
      if (reader.offset < end && readUint8(reader)) {
        ...
        target.filterEffectsMasks[
          target.filterEffectsMasks.length - 1
        ].extra = { top, left, bottom, right, compressionMode, data }
      }
...
addHandler(
  ...
  "artd",
  target => target.artboards !== undefined,
  (reader, target, left) => {
    const desc = readVersionAndDescriptor(reader)
    target.artboards = {
      count: desc["Cnt "],
...
addHandler(
  ...
  (reader, target, left) => {
    ...
    if (desc.enableCompCoreThreads)
      target.compositorUsed.enableCompCoreThreads = enumValue(
        desc.enableCompCoreThreads
      )
...
addHandler(
  "extn",
  target => target._extn !== undefined,
  (reader, target) => {
    const desc = readVersionAndDescriptor(reader)
    // console.log(require('util').inspect(desc, false, 99, true));

    if (MOCK_HANDLERS) target._extn = desc
  },
  (writer, target) => {
