        ...
        html += `
          <div id="trait${trait.id}" class="trait-section">
            <div class="trait-header">
              <h2>TRAIT ${trait.position}${trait.isUserAssignedName && trait.name ? ` - ${escapeHtml(trait.name)}` : ''}</h2>
            ...
            <input type="text" id="trait${trait.id}-name" placeholder="Trait ${trait.position}" ${trait.isUserAssignedName ? `value="${escapeHtml(trait.name)}"` : ''}>
            ...
            html += `
              <div class="variation-container" data-trait-id="${trait.id}" data-variation-id="${variant.id}">
                <div class="variation-image-wrapper${trait.selected === trait.variants.indexOf(variant) ? ' selected' : ''}">
                  <img src="${variant.url}" alt="${escapeHtml(variant.name)}" class="variation">
                </div>
                <div class="variation-filename">${escapeHtml(variant.name)}</div>
              </div>`;
          });
        } else {
          html += `<p>No variants added yet.</p>`;
        }
        html += `</div></div>`;
      });
      html += '</div>';
    ...
    function setupTraitListeners(traitId) {
      ...
      if (nameInput) {
        nameInput.addEventListener('input', () => {
          ...
          title.textContent = `TRAIT ${trait.position}${trait.isUserAssignedName && trait.name ? ` - ${escapeHtml(trait.name)}` : ''}`;
        });
      }
    ...
    function selectVariation(traitId, variationId) {
      ...
      const previewWidth = previewDiv.clientWidth || 600; // Default to 600 if not found
      ...
      // Clear the preview div and use SampleRenderer
      previewDiv.innerHTML = '';
      const images = traitImages
        .map((img, idx) => ({
          img,
          position: TraitManager.getAllTraits()[idx].position,
          zIndex: TraitManager.getAllTraits().length - TraitManager.getAllTraits()[idx].position + 1
        }))
        .sort((a, b) => a.position - b.position)
        .map(item => ({
          img: item.img,
          position: {
            left: parseFloat(item.img.style.left || 0) / (parseFloat(item.img.parentElement?.clientWidth || 600) / 600),
            top: parseFloat(item.img.style.top || 0) / (parseFloat(item.img.parentElement?.clientWidth || 600) / 600)
          },
          zIndex: item.zIndex
        }));

      SampleRenderer(previewDiv, previewWidth, images, 600);
      ...
      magnifyEmoji.addEventListener('click', () => {
        ...
        const sortedImages = traitImages
          ...
          .map(item => ({
            img: item.img,
            position: {
              left: parseFloat(item.img.style.left || 0) / scale,
              top: parseFloat(item.img.style.top || 0) / scale
            },
            zIndex: item.zIndex
          }));

        SampleRenderer(enlargedPreview, enlargedSize, sortedImages, 600);
    ...
    function getPreviewSamplesContent() {
      ...
      sampleData.forEach((sample, i) => {
        html += `<div class="sample-container" id="sample-container-${i}"></div>`;
      });
      html += `</div></div>`;

      // After rendering the HTML, use SampleRenderer for each sample container
      setTimeout(() => {
        sampleData.forEach((sample, i) => {
          const container = document.getElementById(`sample-container-${i}`);
          if (container) {
            const images = sample.map(item => {
              const trait = TraitManager.getTrait(item.traitId);
              const variant = trait.variants.find(v => v.id === item.variantId);
              const img = document.createElement('img');
              img.src = variant.url;
              img.onerror = () => {
                console.error(`Failed to load image for sample ${i + 1} - trait ${trait.id}`);
                img.style.visibility = 'hidden';
              };
              ...
              return {
                ...
                zIndex: TraitManager.getAllTraits().length - trait.position + 1
              };
            });
            SampleRenderer(container, squareSize, images, 600);
            container.addEventListener('click', () => {
              sample.forEach(item => selectVariation(item.traitId, item.variantId));
            });
          }
        });
      }, 0);
