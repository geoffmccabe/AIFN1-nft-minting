  ...
  function removeTrait(traitIndex) {
    ...
    yesButton.addEventListener('click', () => {
      ...
      for (let i = 0; i < traits.length; i++) {
        const oldTraitSection = document.getElementById(`trait${i + 2}`);
        if (oldTraitSection) {
          oldTraitSection.id = `trait${i + 1}`;
          const title = oldTraitSection.querySelector('h2');
          title.textContent = `Trait ${i + 1}`;
          const nameInput = oldTraitSection.querySelector(`#trait${i + 2}-name`);
          nameInput.id = `trait${i + 1}-name`;
          const fileInput = oldTraitSection.querySelector(`#trait${i + 2}-files`);
          fileInput.id = `trait${i + 1}-files`;
          const grid = oldTraitSection.querySelector(`#trait${i + 2}-grid`);
          grid.id = `trait${i + 1}-grid`;
          const upArrow = oldTraitSection.querySelector(`.up-arrow`);
          upArrow.setAttribute('data-trait', `${i + 1}`);
          const downArrow = oldTraitSection.querySelector(`.down-arrow`);
          downArrow.setAttribute('data-trait', `${i + 1}`);
          const addTraitBtn = oldTraitSection.querySelector(`.add-trait`);
          addTraitBtn.setAttribute('data-trait', `${i + 1}`);
          const removeTraitBtn = oldTraitSection.querySelector(`.remove-trait`);
      ...
      Object.keys(variantHistories).forEach(key => {
        ...
        if (newIndex >= traitIndex) {
          newVariantHistories[`${newIndex}-${variationName}`] = variantHistories[key];
        } else if (parseInt(oldIndex) < traitIndex) {
